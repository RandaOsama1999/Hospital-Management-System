 ...
 * @author Randa
 */
public class GuiDoctorChanges extends JFrame{
    ...
    public GuiDoctorChanges()
    {

        setSize(10000,10000);
        btn1=new JButton ("Add Dr");
        btn2=new JButton ("Search, Edit or delete Dr");
        btn3=new JButton ("Display Dr");
        
        try {
            setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("C:\\Users\\rodyr\\Desktop\\Hospital pics\\dr4.jpg")))));
        } catch (IOException ex) {
            Logger.getLogger(GuiDoctorChanges.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private class dosave implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (ae.getSource().equals(btn1)){
                GuiAddDoctor a=new GuiAddDoctor();
                a.setVisible(true);
                a.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                dispose();
            }
            else if (ae.getSource().equals(btn2)){
                GuiSearchDoctor s=new GuiSearchDoctor();
                s.setVisible(true);
                s.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                dispose();
          }
          else if (ae.getSource().equals(btn3)){
                GuiDisplayDoctor di=new GuiDisplayDoctor();
                di.setVisible(true);
