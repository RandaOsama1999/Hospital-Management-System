 ...
 * @author Randa
 */
public class GuiNurseChanges extends JFrame{
    ...
    public GuiNurseChanges()
    {

        setSize(10000,10000);
        btn1=new JButton ("Add Nurse");
        btn2=new JButton ("Search, Edit or delete Nurse");
        btn3=new JButton ("Display Nurse");
        
        try {
            setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("C:\\Users\\rodyr\\Desktop\\Hospital pics\\dr4.jpg")))));
        } catch (IOException ex) {
            Logger.getLogger(GuiNurseChanges.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private class dosave implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (ae.getSource().equals(btn1)){
                GuiAddNurse a=new GuiAddNurse();
                a.setVisible(true);
                a.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                dispose();
            }
            else if (ae.getSource().equals(btn2)){
                GuiSearchNurse s=new GuiSearchNurse();
                s.setVisible(true);
                s.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                dispose();
          }
          else if (ae.getSource().equals(btn3)){
                GuiDisplayNurse di=new GuiDisplayNurse();
                di.setVisible(true);
