 ...
 * @author Randa
 */
public class GuiRecepChanges extends JFrame{
    ...
    public GuiRecepChanges()
    {

        setSize(10000,10000);
        btn1=new JButton ("Add Receptionist");
        btn2=new JButton ("Search, Edit or delete Receptionist");
        btn3=new JButton ("Display Receptionist");
        
        try {
            setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("C:\\Users\\rodyr\\Desktop\\Hospital pics\\dr4.jpg")))));
        } catch (IOException ex) {
            Logger.getLogger(GuiRecepChanges.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private class dosave implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (ae.getSource().equals(btn1)){
                GuiAddReceptionists a=new GuiAddReceptionists();
                a.setVisible(true);
                a.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                dispose();
            }
            else if (ae.getSource().equals(btn2)){
                GuiSearchReceptionists s=new GuiSearchReceptionists();
                s.setVisible(true);
                s.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                dispose();
          }
          else if (ae.getSource().equals(btn3)){
                GuiDisplayReceptionists di=new GuiDisplayReceptionists();
                di.setVisible(true);
